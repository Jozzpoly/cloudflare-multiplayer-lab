...
const BASE_URL = "https://cloudflare-multiplayer-lab-staging.jozzpoly.workers.dev";
...
try {
  ...
  await waitFor("staging A2R bootstrap", async () => {
    const s = await state(sessionId);
    return s.bootStatus?.includes("ws0-a2r-local-box3d-v2") && s.bootStatus?.includes("box3d.js 0.1.1 inline ready") && s.enterDisabled === false;
  });
  await driver(`/session/${sessionId}/execute/sync`, {
    method: "POST",
    body: { script: `document.querySelector('#enter').click(); return true;`, args: [] },
  });

  const baseline = await waitFor("staging A2R live baseline", async () => {
    const s = await state(sessionId);
  ...
  const exercised = await waitFor("staging browser contact", async () => {
    const s = await state(sessionId);
    const local = /^(\d+) steps · (\d+) dropped$/.exec(s.local || "");
    ...
    if (s.notice && !s.notice.includes("hidden")) {
      // notice text is empty in the healthy path; non-empty is retained below for diagnostics.
    }
    return s;
  }, 25_000);

  const productionCanary = await fetch("https://cloudflare-multiplayer-lab.jozzpoly.workers.dev/a2r-containment-canary.txt", { cache: "no-store" });
  if (productionCanary.status !== 404) throw new Error(`production canary drifted during browser smoke: ${productionCanary.status}`);

  console.log(
    `A2R STAGING BROWSER SMOKE PASS · baseline prop Δ ${baseline.propDelta} → exercised ${exercised.propDelta} · ` +
    `player Δ ${exercised.playerDelta} · ${exercised.local} · ack ${exercised.ack} · phase ${exercised.phase} · ` +
    `production canary 404`,
  );
} finally {
