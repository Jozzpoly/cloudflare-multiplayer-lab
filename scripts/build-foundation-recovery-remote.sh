...
if (trigger.revision !== "multiplayer-foundation-remote-recovery-trigger-v1") {
  throw new Error("remote recovery trigger revision mismatch");
}
if (!["idle", "seed", "resume"].includes(trigger.phase)) {
...
} else if (trigger.phase === "seed") {
  if (trigger.campaignId === "unarmed" || trigger.expectedPreviousInstanceNonce !== null) {
    throw new Error("seed remote recovery trigger fields are invalid");
  }
} else if (
