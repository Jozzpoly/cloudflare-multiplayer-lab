...
const WORLD_EPOCH = "client-box3d-byte-runtime-smoke-epoch-1";
const CAPACITY = 6;
...
assert(Number.isInteger(reportedSeedBytes) && reportedSeedBytes > 0, "seed-only byte recording must be non-empty");
const seedBytes = b3.b3Recording_CopyData(recording);
assert(seedBytes instanceof Uint8Array, "pinned i4 seed CopyData must return Uint8Array");
assert.equal(seedBytes.byteLength, reportedSeedBytes, "seed CopyData length mismatch");
...
// Treat the envelope as a wire payload rather than sharing object identity with authority.
const transportedEnvelope = JSON.parse(JSON.stringify(runtimeEnvelope)) as FoundationClientRuntimeBootstrapEnvelope;
...
assert.equal(u32Hex(b3.b3Bytes_Fnv1a32(hydrated.executionSeedBytes)), seedChecksum);
assert.equal(hydrated.inputLedger.activeOwners().length, 6);

const corruptedEnvelope = JSON.parse(JSON.stringify(runtimeEnvelope)) as FoundationClientRuntimeBootstrapEnvelope;
...
assert.throws(
  ...
  /checksum mismatch/,
  "runtime bootstrap must reject a single-byte seed mutation before Box3D hydration",
);
...
assert.equal(
  firstDifference,
  null,
  `byte-transport client Box3D runtime diverged from uninterrupted authority: ${JSON.stringify(firstDifference)}`,
);
assert.equal(client.boundaryTick, endTick);

destroyFoundationBox3DClientRuntime(client);

console.log("MULTIPLAYER_FOUNDATION_CLIENT_BOX3D_BYTE_RUNTIME_PASS", JSON.stringify({
