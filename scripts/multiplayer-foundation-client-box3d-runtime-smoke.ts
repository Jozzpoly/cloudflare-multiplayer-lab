...
function intendedInput(actorId: string, targetTick: number): { x: number; z: number; jump: false } {
  ...
  if (targetTick < 25) return { x: 0, z: 0, jump: false };
  if (targetTick < 260) {
  ...
  const angle = ordinal * 1.0471975512 + (targetTick - 260) * 0.035;
...
function createRuntime(): RuntimeState {
  ...
  for (const prop of WORLD_V0_PROP_LAYOUT) {
    props.set(prop.id, createPropBody(world, prop.id, prop.position));
  }
...
for (let tick = 0; tick <= CHECKPOINT_TICK; tick += 1) {
  checkpointGuard = advanceSource(source, tick);
}
...
assert(maxPropDisplacement > 0.25, `checkpoint fixture must contain material actor→prop contact, max displacement=${maxPropDisplacement}`);
...
for (const netEntityId of topologyAtCheckpoint.entityOrder) {
  const actor = source.actorsById.get(netEntityId);
  const prop = source.props.get(netEntityId);
  const body = actor?.body ?? prop;
...
const bootstrapEnvelope = createFoundationClientBootstrap({
  ...
  entityStates: topologyAtCheckpoint.entityOrder.map((netEntityId) => ({
    netEntityId,
  ...
  inputBaselines: rosterAtCheckpoint.actors.map((actor) => ({
    ...
    ...intendedInput(actor.actorId, CHECKPOINT_TICK),
  })),
});
...
assert.equal(b3.b3Recording_GetSize(recording) > 0, true, "seed-only client recording must contain internal Box3D state");
...
assert.throws(
  ...
  /body count mismatch|manifest drift/,
  "runtime hydration must fail closed on an incomplete semantic body manifest",
);
...
assert.equal(client.boundaryTick, CHECKPOINT_TICK);
...
assert.equal(
  captureFoundationBox3DClientGuard(client, bodyValues).packed,
  checkpointGuard.packed,
  "hydrated client runtime must be exact at checkpoint boundary",
);
...
assert.equal(
  firstDifference,
  null,
  `client Box3D runtime diverged from uninterrupted authority: ${JSON.stringify(firstDifference)}`,
);
assert.equal(client.boundaryTick, END_TICK);
...
console.log("MULTIPLAYER_FOUNDATION_CLIENT_BOX3D_RUNTIME_PASS", JSON.stringify({
  ...
  endTick: END_TICK,
  exactContinuationTicks: END_TICK - CHECKPOINT_TICK,
  activeActors: 6,
  dynamicEntities: topologyAtCheckpoint.entityOrder.length,
  seedBodyCount: expectedBodyNames.length,
  maxPropDisplacementAtCheckpoint: Number(maxPropDisplacement.toFixed(4)),
