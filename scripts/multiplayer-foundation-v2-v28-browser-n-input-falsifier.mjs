...
const OUTPUT = process.env.MW_MF6_N_INPUT_OUTPUT || "mf6-browser-n-input-limit.json";
...
const result = { verdict: "MF6_V28_BROWSER_N_INPUT_FALSIFIER_FAIL", run: RUN, generatedAt: new Date().toISOString() };
...
try {
  ...
  // Control phase: move only the one remote that the current V28 client explicitly tracks.
  tracked.feed.stop();
  ...
  assert(!singleRemote.runtimeFailed, `single tracked remote failed: ${singleRemote.runtimeFailureReason}`);
  assert(singleRemote.metrics.guardMismatches === 0, "single tracked remote produced exact-state mismatch");

  // Falsifier phase: add independent motion from a second remote. Current V28 still
  // resolves/applies only self + one selected remote, so exact state should expose
  // the missing N-peer input dimension rather than hiding it behind presentation.
  second.feed.stop();
  second.feed = startFeed(second, [-0.72, 0]);
  feeds.push(second.feed);

  const divergence = await waitFor(
    async () => {
      const e = await cdp.eval(browserSession, "window.__sharedYardV0Evidence()");
      return e?.runtimeFailed &&
        e.metrics?.guardMismatches >= 1 &&
        String(e.runtimeFailureReason || "").includes("FOUNDATION_STATE_DIVERGENCE")
        ? e
        : false;
    },
    "second remote fixed-2 boundary",
    20_000,
  );

  Object.assign(result, {
    verdict: "MF6_V28_BROWSER_N_INPUT_LIMIT_CONFIRMED",
    worldEpoch: epoch,
    ...
    trackedRemote: {
      ...
      controlGuardMatchesBefore: before.metrics.guardMatches,
      controlGuardMatchesAfter: singleRemote.metrics.guardMatches,
      controlGuardMismatches: singleRemote.metrics.guardMismatches,
      controlBoundary: singleRemote.localBoundaryTick,
    },
    secondRemote: {
    ...
    observedLimit: {
      runtimeFailureReason: divergence.runtimeFailureReason,
      firstStateMismatch: divergence.metrics.firstStateMismatch,
      guardMismatches: divergence.metrics.guardMismatches,
      boundaryTick: divergence.localBoundaryTick,
    },
    interpretation: "Current V28 browser exact replica composes six bodies and one tracked remote input, but fails exactness when a second remote independently drives authority state. This is the intended fixed-2 input/reconciliation seam for the next implementation tranche.",
    nonClaim: "This falsifier does not specify the final N-peer prediction architecture and does not qualify multiplayer performance, impairment, presentation, deployed edge, or human 3-6 play.",
  });
  writeFileSync(OUTPUT, JSON.stringify(result, null, 2));
  console.log("MF6_V28_BROWSER_N_INPUT_EVIDENCE", JSON.stringify(result));
