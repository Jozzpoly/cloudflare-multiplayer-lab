...
assert(!rootBindings.has("SHARED_YARD_V0"), "World V0 binding must remain staging-only");
...
assert(stagingBindings.size === rootBindings.size + 2, "staging must add exactly isolated F5 + Shared Yard V0 bindings");
assert(stagingBindings.get("WORLD_SLICE_F5") === "WorldSliceF5", "staging WORLD_SLICE_F5 binding missing or wrong");
assert(stagingBindings.get("SHARED_YARD_V0") === "SharedYardV0", "staging SHARED_YARD_V0 binding missing or wrong");
...
assert(!rootExports.SharedYardV0, "Shared Yard V0 export must not mutate root lifecycle");
...
assert(stagingExports.SharedYardV0?.type === "durable-object", "staging Shared Yard V0 export missing");
assert(stagingExports.SharedYardV0?.storage === "sqlite", "staging Shared Yard V0 Durable Object must use sqlite storage");
...
assert(!rootWorkerFirst.has("/world-v0/ws"), "World V0 worker-first route must remain staging-only");
assert(stagingWorkerFirst.has("/world-v0/ws"), "staging run_worker_first missing /world-v0/ws");
...
assert(scripts["test:world-v0-protocol"] === "node --experimental-strip-types scripts/world-v0-protocol-smoke.ts", "World V0 protocol smoke missing");
...
assert(String(scripts.check || "").includes("test:world-v0-protocol"), "main check must exercise World V0 protocol smoke");
...
console.log(
  `World V0 staging contract PASS · root ${wrangler.name} frozen at ${rootBindings.size} DO bindings · staging ${staging.name} has isolated F5 + SHARED_YARD_V0`,
);
