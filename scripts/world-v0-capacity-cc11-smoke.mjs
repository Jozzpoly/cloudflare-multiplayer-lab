...
const OUTPUT = process.env.MW_WORLD_V0_CAPACITY_CC11_OUTPUT || "world-v0-capacity-cc11-evidence.json";
const DEBUG_PORT = Number(process.env.MW_WORLD_V0_CAPACITY_DEBUG_PORT || 9441);
const TIMEOUT_MS = Number(process.env.MW_WORLD_V0_CAPACITY_TIMEOUT_MS || 900_000);
...
const options = {
  counts: parseCounts(process.env.MW_WORLD_V0_CAPACITY_COUNTS, [384, 512, 640, 768, 896]),
  ...
  stopAfterFirstBroken: process.env.MW_WORLD_V0_CAPACITY_STOP_AFTER_BREAK !== "0",
};
...
const profile = mkdtempSync(join(tmpdir(), "mw-world-v0-cc11-"));
const stderr = [];
...
let cdp = null;
const envelope = {
  verdict: "WORLD_V0_CAPACITY_CC11_HARNESS_FAIL",
...
try {
  ...
  assert(boot.ready === true, `CC1.1 boot failed: ${boot.bootError || "unknown"}`);

  const expression = `window.__worldV0CapacityCC11.runSuite(${JSON.stringify(options)})`;
  const started = Date.now();
  const evidencePromise = cdp.evaluate(sessionId, expression);
  const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`CC1.1 exceeded ${TIMEOUT_MS} ms harness timeout`)), TIMEOUT_MS));
  const evidence = await Promise.race([evidencePromise, timeoutPromise]);
  assert(evidence?.verdict === "WORLD_V0_CAPACITY_CC11_COMPLETE", `unexpected CC1.1 verdict ${evidence?.verdict}`);

  envelope.verdict = "WORLD_V0_CAPACITY_CC11_HARNESS_COMPLETE";
  envelope.boot = boot;
  envelope.elapsedMs = Date.now() - started;
  envelope.evidence = evidence;
  writeFileSync(OUTPUT, `${JSON.stringify(envelope, null, 2)}\n`);

  const boundaryText = Object.entries(evidence.boundaries || {}).map(([key, value]) => {
    const good = value.lastKnownGood?.count ?? "none";
    const broken = value.firstBroken ? `${value.firstBroken.count}:${value.firstBroken.classification}` : "none";
    return `${key}[good=${good},broken=${broken}]`;
  }).join(" ");
  console.log(`${envelope.verdict} · ${version} · ${boundaryText}`);
} catch (error) {
