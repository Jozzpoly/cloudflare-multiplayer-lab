...
async function waitForRoom(id, predicate, label, timeout = 8000) {
  const started = Date.now();
  let last = null;
  while (Date.now() - started < timeout) {
...
async function openDrivingPeer(player) {
  ...
  function fillFuture(boundaryTick) {
    ...
    while (nextTarget <= horizon && ws.readyState === WebSocket.OPEN) {
      const remaining = horizon - nextTarget + 1;
...
const profile = mkdtempSync(join(tmpdir(), "mw-r0b-freeze-recovery-"));
...
try {
  ...
  await cdp.evaluate(sessionId, `(() => { document.querySelector("#callsign").value = "R0B-Frozen"; document.querySelector("#run").value = "yard-1"; document.querySelector("#enter").click(); return true; })()`);
  ...
  peerA = await openDrivingPeer("R0B-Driver-A");
  ...
  assert(liveA.protocolStartTick !== null, "initial epoch did not start");
  assert(liveA.metrics.guardMismatches === 0, `initial guard mismatch ${JSON.stringify(liveA.metrics.firstStateMismatch)}`);

  await cdp.call("Page.setWebLifecycleState", { state: "frozen" }, sessionId);

  const ended = await Promise.race([
    peerA.epochEnded,
    sleep(7000).then(() => { throw new Error("input lease epoch end timeout while browser frozen"); }),
  ]);
  assert(ended.reason === "input_lease_expired:actor:0", `unexpected frozen epoch end ${ended.reason}`);
  assert(ended.worldEpoch === epochA, `ended epoch mismatch ${ended.worldEpoch} != ${epochA}`);
  peerA = null;

  const emptyWhileFrozen = await waitForRoom(
    "yard-1",
    (room) => room.occupancy === 0 && room.state === "empty" && room.worldEpoch === null,
    "room did not become empty while renderer frozen",
    5000,
  );
  assert(emptyWhileFrozen.joinable === true, `empty room not joinable ${JSON.stringify(emptyWhileFrozen)}`);

  await sleep(700);
  const stillEmpty = await waitForRoom(
    "yard-1",
    (room) => room.occupancy === 0 && room.state === "empty" && room.worldEpoch === null,
    "frozen browser unexpectedly reconnected",
    1500,
  );
