...
const v18SnapshotBlock = `  await cdp.eval(sessionId, "window.__mwV17SetOrderedInboundDelay(0)");\n  await sleep(Math.max(1000, LATENCY_MS + 450));\n  const v18Snapshot = await cdp.eval(sessionId, "(() => { window.__mwV18FlushCorrectionBatching('final-snapshot'); return { correctionVectors: window.__mwRenderProbeV6ReadCorrections(), predictionCeilingV15: window.__mwV15ReadPredictionCeiling(), orderedInboundV17: window.__mwV17ReadOrderedInbound(), correctionBatchingV18: window.__mwV18ReadCorrectionBatching() }; })()");\n  const correctionVectors = v18Snapshot.correctionVectors;\n  const predictionCeilingV15 = v18Snapshot.predictionCeilingV15;\n  const orderedInboundV17 = v18Snapshot.orderedInboundV17;\n  const correctionBatchingV18 = v18Snapshot.correctionBatchingV18;`;
replaceOnce(v17SnapshotBlock, v18SnapshotBlock, "V18 atomic final flush and snapshot");
replaceOnce(
