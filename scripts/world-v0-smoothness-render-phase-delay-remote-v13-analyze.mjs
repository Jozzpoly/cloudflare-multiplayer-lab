...
const output = process.argv[3] || "world-v0-smoothness-phase-delay-remote-v13-analysis.json";
if (!input) throw new Error("usage: node world-v0-smoothness-render-phase-delay-remote-v13-analyze.mjs <capture.json> [output.json]");
const source = JSON.parse(readFileSync(input, "utf8"));
const timeline = source.confirmedTimelineV11 || {};
const presentation = source.phaseDelayRemoteV13 || {};
...
const clockToPresentationLag = presentationSamples.map((entry) => entry.presentationClockBoundary - entry.presentationBoundary);
const targetTrackingErrorTicks = presentationSamples.map((entry) => Math.abs(entry.targetBoundary - entry.presentationBoundary));
const localMinusPresentation = presentationSamples.map((entry) => Number.isFinite(entry.localBoundary) ? entry.localBoundary - entry.presentationBoundary : NaN);
const spatialOffsets = presentationSamples.map((entry) => entry.spatialOffset);
const phaseFractions = presentationSamples.map((entry) => entry.targetBoundary - Math.floor(entry.targetBoundary));
...
const signature = {
  ...
  phaseDelayConfigured: Number.isFinite(delayTicks),
  phaseDelayPresentationEnabled: presentation.enabled === true,
  phaseDelayPresentationSampled: presentationSamples.length > 0,
  visibleRemoteIntervalsSampled: intervals.length > 0,
};
const comparable = Object.values(signature).every(Boolean);

const result = {
  revision: "world-v0-smoothness-phase-delay-remote-v13-analysis-v1",
  sourceRevision: source.revision ?? null,
  status: "test-only continuous authority-phase confirmed-anchor remote presentation characterization; no Owner-ready latency/smoothness SLO asserted",
  delayTicks,
  ...
  presentation: {
    ...
    authorityClockMinusPresentationBoundaryTicks: summary(clockToPresentationLag),
    targetTrackingErrorTicks: summary(targetTrackingErrorTicks),
    localMinusPresentationBoundaryTicks: summary(localMinusPresentation),
    targetFraction: summary(phaseFractions),
  ...
  signature,
  verdict: comparable ? "PHASE_DELAY_REMOTE_PRESENTATION_CHARACTERIZED" : "PHASE_DELAY_REMOTE_PRESENTATION_NOT_COMPARABLE",
};
