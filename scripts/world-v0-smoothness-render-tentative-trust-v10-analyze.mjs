...
const output = process.argv[3] || "world-v0-smoothness-render-tentative-trust-v10-analysis.json";
if (!input) throw new Error("usage: node world-v0-smoothness-render-tentative-trust-v10-analyze.mjs <evidence.json> [output.json]");

const evidence = JSON.parse(readFileSync(input, "utf8"));
const policy = evidence.tentativeTrustV10;
if (!policy || !Number.isFinite(Number(policy.alpha))) throw new Error("V10 tentative trust evidence missing");
const vectors = evidence.correctionVectors || [];
const decisions = policy.corrections || [];
if (vectors.length !== decisions.length) throw new Error(`V10/V6 correction evidence count mismatch ${vectors.length} != ${decisions.length}`);
...
const aligned = vectors.map((vector, index) => {
  ...
  for (const key of ["reason", "targetTick", "boundaryBefore"]) {
    if (vector[key] !== decision[key]) throw new Error(`V10/V6 correction alignment mismatch at ${index} field=${key}`);
  }
...
const remoteCorrections = aligned.filter((event) => event.remoteCorrection > 1e-9);
const byReason = {};
for (const reason of [...new Set(remoteCorrections.map((event) => event.reason))].sort()) {
  const events = remoteCorrections.filter((event) => event.reason === reason);
  byReason[reason] = {
    correctionMeters: summarize(events.map((event) => event.remoteCorrection)),
    rewind: summarize(events.map((event) => event.rewind)),
...
for (const event of remoteCorrections) {
  const exactKey = `${event.reason}|${event.rewind}`;
  if (!exactGroups.has(exactKey)) exactGroups.set(exactKey, []);
  exactGroups.get(exactKey).push(event);
  const bucketKey = `${event.reason}|${bucketFor(event.rewind)}`;
...
const exactReasonRewind = Object.fromEntries([...exactGroups.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([key, events]) => [key, summarizeGroup(events)]));
const reasonRewindBuckets = Object.fromEntries([...bucketGroups.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([key, events]) => [key, summarizeGroup(events)]));

const result = {
  revision: "world-v0-smoothness-render-tentative-trust-v10-analysis-v1",
  sourceRevision: evidence.revision,
  status: "tentative peer-future trust characterization; compare matched reason+rewind and total correction burden",
  ...
  alignment: { correctionVectors: vectors.length, policyCorrections: decisions.length, aligned: true },
  totalRemoteCorrection: summarize(remoteCorrections.map((event) => event.remoteCorrection)),
  byReason,
  exactReasonRewind,
  reasonRewindBuckets,
  samples: remoteCorrections,
};

result.signature = {
  policyResolved: Number.isFinite(result.alpha),
  ...
  remoteCorrectionObserved: result.totalRemoteCorrection.count > 0,
};
result.verdict = Object.values(result.signature).every(Boolean)
  ? "TENTATIVE_TRUST_POLICY_CHARACTERIZED"
  : "TENTATIVE_TRUST_POLICY_NOT_COMPARABLE";
