...
const WS_URL = "wss://cloudflare-multiplayer-lab-staging.jozzpoly.workers.dev/world-v0/ws";
const POLL_TIMEOUT_MS = 120_000;
const SMOKE_TIMEOUT_MS = 25_000;
const EXPECTED_DYNAMIC_BODIES = 14;
const EXPECTED_STATE_COMPONENTS = 13;
const EXPECTED_GUARD_LENGTH = EXPECTED_DYNAMIC_BODIES * EXPECTED_STATE_COMPONENTS * 8;
...
async function waitForExpectedStaging() {
  ...
  while (Date.now() - started < POLL_TIMEOUT_MS) {
    try {
      const [build, app, ping] = await Promise.all([
        fetchText(`${STAGING}/world-v0/build-contract.js`),
        fetchText(`${STAGING}/world-v0/app.js`),
      ...
      if (build.response.ok && app.response.ok && ping.response.ok && last.hasSimBuild && last.hasUiRevision) {
        console.log(`World V0 staging assets ready · ${JSON.stringify(last)}`);
        return;
      }
    } catch (error) {
  ...
  throw new Error(`World V0 staging did not reach expected browser build within ${POLL_TIMEOUT_MS}ms · ${JSON.stringify(last)}`);
}

async function assertProductionIsolation(label) {
  const response = await fetch(`${PRODUCTION}/world-v0/build-contract.js`, {
    cache: "no-store",
...
function sendProbeRecords(peer, firstTick, input, count = 30) {
  if (peer.sentProbe) return;
  assert(count % 2 === 0, "probe record count must align to batch size 2");
...
async function runTwoPeerSharedYardSmoke() {
  ...
  const cleanup = () => {
    ...
    for (const peer of peers) {
      try { peer.ws.close(1000, "world_v0_cloud_smoke_complete"); } catch { /* best effort */ }
    }
  };
  ...
  return await new Promise((resolve, reject) => {
    ...
    const maybeSendProbe = () => {
      ...
      if (minBoundary < protocolStartTick - 2) return;
      sendProbeRecords(peers[0], protocolStartTick, { x: 1, z: 0 });
      sendProbeRecords(peers[1], protocolStartTick, { x: -1, z: 0 });
    };
    ...
    const maybePass = () => {
      ...
      try {
        for (const peer of peers) {
          ...
          assert(peer.acceptedRecords >= 30, `${peer.playerId}: accepted ${peer.acceptedRecords}`);
          assert(peer.relayedRecords >= 30, `${peer.playerId}: relayed ${peer.relayedRecords}`);
          assert(peer.consumedFreshSelf >= 20, `${peer.playerId}: too few fresh self inputs ${peer.consumedFreshSelf}`);
          assert(peer.consumedFreshRemote >= 20, `${peer.playerId}: too few fresh remote inputs ${peer.consumedFreshRemote}`);
          assert(peer.finiteSnapshots >= 6, `${peer.playerId}: too few finite snapshots ${peer.finiteSnapshots}`);
          ...
          assert(peer.maxPropDisplacement > 0.05, `${peer.playerId}: Shared Yard props did not move meaningfully (${peer.maxPropDisplacement})`);
        }
        ...
        assert(sharedGuardSamples >= 5, `too few identical authority guard samples across peers ${sharedGuardSamples}`);
        const result = {
          runKey,
    ...
    timeout = setTimeout(() => fail(new Error(`World V0 staging smoke timeout · ${JSON.stringify(peers.map((peer) => ({
      playerId: peer.playerId,
        ...
        try {
          ...
          if (message.type === "world_v0_snapshot") {
            ...
            for (const prop of message.props || []) {
              ...
              if (!initial) continue;
              peer.maxPropDisplacement = Math.max(
                peer.maxPropDisplacement,
                Math.hypot(prop.position[0] - initial[0], prop.position[1] - initial[1], prop.position[2] - initial[2]),
              );
            }
          ...
          if (message.type === "world_v0_batch_ack") {
            ...
            for (const record of message.records || []) {
              ...
              else if (record.status !== "duplicate_same") throw new Error(`${peer.playerId}: unexpected record status ${record.status} at ${record.targetTick}`);
            }
            maybeSendProbe();
            return;
          }
          ...
          if (message.type === "world_v0_welcome") {
            ...
            if (index === 0) {
              ...
              peer.ws.send(JSON.stringify({
                type: "world_v0_ready",
                ...peer.identity,
                worldEpoch: `${peer.identity.worldEpoch}-wrong`,
              }));
            }
            return;
          }
          if (message.type === "world_v0_epoch_ended") peer.epochEnded = message;
          if (message.type === "world_v0_error" && message.error !== "world_identity_mismatch") {
            throw new Error(`unexpected identity smoke server error ${message.error}`);
          }
        } catch (error) {
          fail(error);
        }
...
await assertProductionIsolation("before World V0 staging smoke");
await waitForExpectedStaging();
const sharedYard = await runTwoPeerSharedYardSmoke();
const identityMismatch = await runIdentityMismatchSmoke();
await assertProductionIsolation("after World V0 staging smoke");

console.log(`WORLD V0 STAGING CLOUD SMOKE PASS · ${JSON.stringify({ sharedYard, identityMismatch })}`);
