...
renderer.domElement.addEventListener("lostpointercapture", (event) => {
  if (event.pointerType === "touch") endCameraPointer(event);
});`,
  "touch-only-canvas-lostpointercapture"
);

fs.writeFileSync(path, source);
console.log("WORLD_V0_TWO_PHONE_INTERRUPTION_V2_TRANSFORM_APPLIED", JSON.stringify({ path }));
